...
(async function main() {
    function getData() {
        const data = JSON.parse(
            fs.readFileSync(`${path}data.json`, { encoding: "utf8", flag: "r" })
        );
    ...
    async function setUpWizard() {
        ...
        while (true) {
            ...
            projects = await (async () => {
                try {
                    projects = await new TodoistApi(api_key).getProjects();
                } catch (error) {
                    return null;
                }
                return projects;
            ...
            await list_tasks(async (tasks, confData, incTime) => {
                ...
                const data = (() => {
                    ...
                    if (inpArgs.length === 1 && inpArgs[0] === "help") {
                        ...
                        if (
                            "name description date time priority done".includes(inpArgs[1])
                        ) {
                            switch (inpArgs[1]) {
                                case "name":
                                    inpArgs[1] = "content";
                ...
                if (data && data !== "help") {
                    ...
                    if (toChange === "done") {
                        await api.closeTask(data.task_id);
                        return;
                        ...
                        if (data.val[0] === "" || data.val === []) {
                            ...
                            await api.updateTask(data.task_id, {
                                dueDatetime: `${
                                    (
                                        await api.getTask(data.task_id)
                                    ).due.date
                                }T${incTime(data.val[0], -2)}:00Z`,
                            });
                            return;
                        }
                    }
